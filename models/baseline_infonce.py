...
class Baseline(nn.Module):
    def __init__(self, num_classes=None, backbone="resnet50", drop_last_stride=False, pattern_attention=False, modality_attention=0, mutual_learning=False, **kwargs):
        ...
        # InfoNCE
        self.infonce_temp = kwargs.get('infonce_temp', 0.1)
        self.infonce_weight = kwargs.get('infonce_weight', 0.1)
    ...
    def forward(self, inputs, labels=None, **kwargs):
        ...
        if not self.training:
            ...
            return self.train_forward(feats, labels, loss_dp, sub, raw_feat=global_feat, **kwargs)


    def train_forward(self, feat, labels, loss_dp, sub, raw_feat=None, **kwargs):
        metric = {}
        ...
        # 在 feat = self.bn_neck(...) 之前用
        loss_infonce = self.info_nce_cross_modality(raw_feat, labels, sub, temp=self.infonce_temp)

        metric.update({'infonce': loss_infonce.data})

        metric.update({'id': loss_id.data})
        metric.update({'cs': loss_cs.data})
        metric.update({'dp': loss_dp.data})

        loss = loss_id + loss_cs * self.cs_w + loss_dp * self.dp_w + loss_infonce * self.infonce_weight

        return loss, metric

    def info_nce_cross_modality(self, features, labels, sub, temp=0.07):
        features = F.normalize(features, dim=1)

        vis_idx = (sub == 0).nonzero(as_tuple=True)[0]
        ir_idx = (sub == 1).nonzero(as_tuple=True)[0]

        if len(vis_idx) == 0 or len(ir_idx) == 0:
            return torch.tensor(0.0, device=features.device)

        feat_v = features[vis_idx]
        feat_i = features[ir_idx]
        label_v = labels[vis_idx].view(-1, 1)
        label_i = labels[ir_idx].view(-1, 1)

        # visible → infrared
        sim_vi = torch.mm(feat_v, feat_i.t()) / temp
        label_match_vi = (label_v == label_i.t()).float()
        exp_sim_vi = torch.exp(sim_vi)
        log_prob_vi = sim_vi - torch.log(exp_sim_vi.sum(dim=1, keepdim=True) + 1e-8)
        mean_log_prob_pos_vi = (label_match_vi * log_prob_vi).sum(1) / label_match_vi.sum(1).clamp(min=1.0)
        loss_vi = -mean_log_prob_pos_vi.mean()

        # infrared → visible
        sim_iv = torch.mm(feat_i, feat_v.t()) / temp
        label_match_iv = (label_i == label_v.t()).float()
        exp_sim_iv = torch.exp(sim_iv)
        log_prob_iv = sim_iv - torch.log(exp_sim_iv.sum(dim=1, keepdim=True) + 1e-8)
        mean_log_prob_pos_iv = (label_match_iv * log_prob_iv).sum(1) / label_match_iv.sum(1).clamp(min=1.0)
        loss_iv = -mean_log_prob_pos_iv.mean()

        return (loss_vi + loss_iv) / 2.0
